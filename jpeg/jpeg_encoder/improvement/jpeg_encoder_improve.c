...
/*double*/float LDQT[64] = { 16, 11, 10, 16, 24, 40, 51, 61,
...
/*double*/float CDQT[64] = { 17, 18, 24, 47, 99, 99, 99, 99,
...
int cal_size(/*long double*/float val){
    /*long double*/float abs_ = fabsl(val);
    int s = 1;
    int count = 1;
...
int DC_codeword(/*long double*/float * iarray, int y, int x, int * oarray, int length){ // conversion the DC difference to codeword by lookup the huffman table
    int num = 0, size, amplitude, code;
    char * temp;
    /*long double*/float current_value;
    for(int i = 0; i < y; i++){
        for(int j = 0; j < x; j++){
...
int main(){
    ...
    float current_dct_Y[64];
    float current_dct_Cb[64];
    float current_dct_Cr[64];
    float zeros[64]={0};
    float * all_DC_Y;
    float * all_DC_Cb;
    float * all_DC_Cr;
    float * all_difference_DC_Y;
    float * all_difference_DC_Cb;
    float * all_difference_DC_Cr;
    ...
    all_DC_Y  = calloc(block_num_y * block_num_x, sizeof(float)); //initialize with zeros
    all_DC_Cb = calloc(block_num_y * block_num_x, sizeof(float));
    all_DC_Cr = calloc(block_num_y * block_num_x, sizeof(float));
    all_difference_DC_Y  = calloc(block_num_y * block_num_x, sizeof(float)); //initialize with zeros
    all_difference_DC_Cb = calloc(block_num_y * block_num_x, sizeof(float));
    all_difference_DC_Cr = calloc(block_num_y * block_num_x, sizeof(float));
    ...
    for(int y = 0; y < block_num_y; y++){
        for(int x = 0; x < block_num_x; x++){
            ...
            /*long double*/float c_v, c_u, sum_Y, sum_Cb, sum_Cr;
            for(int v = 0; v < 8; v ++){
                c_v = (v==0)? 1./sqrt(2) : 1;
                for(int u = 0; u < 8; u++){
                    c_u = (u==0)? 1./sqrt(2) : 1;
                    ...
                    /*current_dct_Y[v*8+u] = ((int)(c_u*c_v*sum_Y)) >> 2;
                    current_dct_Cb[v*8+u]= ((int)(c_u*c_v*sum_Cb))>> 2;
                    current_dct_Cr[v*8+u]= ((int)(c_u*c_v*sum_Cr))>> 2;*/
                }
            }
    ...
    float temp_AC_Y,temp_AC_Cb,temp_AC_Cr;
    float temp_AC_Y2,temp_AC_Cb2,temp_AC_Cr2;
    for(int i = 0; i < block_num_y; i++){
        for(int j = 0; j < block_num_x; j++){
            if(i==0 && j==0){
                ...
                temp_AC_Y = all_DC_Y[i*block_num_x+j];
                temp_AC_Cb= all_DC_Cb[i*block_num_x+j];
                temp_AC_Cr= all_DC_Cr[i*block_num_x+j];
            }
            else{
                ...
                temp_AC_Y2 = all_DC_Y[i*block_num_x+j];
                temp_AC_Cb2= all_DC_Cb[i*block_num_x+j];
                temp_AC_Cr2= all_DC_Cr[i*block_num_x+j];
                all_DC_Y[i*block_num_x+j] = all_DC_Y[i*block_num_x+j]-temp_AC_Y;
                all_DC_Cb[i*block_num_x+j] = all_DC_Cb[i*block_num_x+j]-temp_AC_Cb;
                all_DC_Cr[i*block_num_x+j] = all_DC_Cr[i*block_num_x+j]-temp_AC_Cr;
                temp_AC_Y = temp_AC_Y2;
                temp_AC_Cb= temp_AC_Cb2;
                temp_AC_Cr= temp_AC_Cr2;
            }
            printf("%d\t", (int)all_DC_Y[i*block_num_x+j]);
    ...
    num_code = DC_codeword(all_DC_Y,block_num_y, block_num_x,codeword,code_size);
    for(int a=0; a<num_code;a++){
        big_codeword[codeword_length+a] = codeword[a];
    }
    codeword_length += num_code;
